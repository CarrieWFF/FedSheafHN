...
def convert_np_to_tensor(state_dict, gpu_id, skip_stat=False, skip_mask=False, model=None):
    ...
    def tensorize(value, requires_grad=True):
        tensor = torch.tensor(value, dtype=torch.float32 if np.issubdtype(value.dtype, np.floating) else torch.int64)
        if requires_grad:
    ...
    for k,v in state_dict.items():
        ...
        if isinstance(v, dict):
            _state_dict[k] = convert_np_to_tensor(v, gpu_id, skip_stat, skip_mask, model)
        elif len(np.shape(v)) == 0:
            _state_dict[k] = tensorize(v, requires_grad=False)
        else:
            _state_dict[k] = tensorize(v)
