...
def split_train(data, dataset, data_path, ratio_train, mode, n_clients):
    ...
    target_dir = os.path.join(data_path, f'{dataset}_{mode}/{n_clients}')
    os.makedirs(target_dir, exist_ok=True)  

    torch_save(target_dir, 'train.pt', {'data': data})
    torch_save(target_dir, 'test.pt', {'data': data})
    torch_save(target_dir, 'val.pt', {'data': data})
